...
public static class ShopECPay
{
            ...
            using (AllInOne oPayment = new AllInOne())
            {
                using (tblOrders orders = new tblOrders())
                {
                    using (tblOrdersDetail ordersDetail = new tblOrdersDetail())
                    {
                        ...
                        if (order != null)
                        {
                            string str_home_url = GetAppConfigValue("ECPayHomeURL");
                            string str_prod_url = "";
                            ...
                            oPayment.ServiceURL = GetAppConfigValue("ECPayServiceURL");
                            //ECPay提供的Hash Key Demo = 5294y06JbISpM5x9
                            oPayment.HashKey = GetAppConfigValue("ECPayHashKey");
                            //ECPay提供的Hash IV Demo = v77hoKGq4kWxNNIS
                            oPayment.HashIV = GetAppConfigValue("ECPayHashIV");
                            //ECPay提供的特店編號 Demo = 2000132
                            oPayment.MerchantID = GetAppConfigValue("ECPayMerchantID");

                            /* 基本參數 */
                            //付款完成通知回傳的網址
                            oPayment.Send.ReturnURL = GetAppConfigValue("ECPayReturnURL");
                            //瀏覽器端返回的廠商網址
                            oPayment.Send.ClientBackURL = GetAppConfigValue("ECPayClientBackURL");
                            //瀏覽器端回傳付款結果網址
                            oPayment.Send.OrderResultURL = GetAppConfigValue("ECPayOrderResultURL");
                            //訂單編號前置碼
                            string ECPayOrderCode = GetAppConfigValue("ECPayOrderCode");
                            //廠商的交易編號
                            ...
                            //訂單的商品資料
                            var detail = ordersDetail.repo.ReadAll(m => m.order_no == str_order_no);
                            if (detail != null && detail.Count() > 0)
                            {
                                foreach (var item in detail)
                                {
                                    str_prod_url = string.Format("{0}/Shop/Detail/{1}", str_home_url, item.product_no);
    ...
    /// <summary>
    /// 讀取 Web.config 中的 appsetting 設定值
    /// </summary>
